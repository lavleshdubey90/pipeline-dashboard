  ...
  const filteredCandidates = useMemo(() => {
    return candidateData.filter(
      (
        ...
        // Filter by stage
        const matchesStage =
          filters.stage === "All" || candidate.status === filters.stage;
        ...
        return (
          matchesSearch && matchesStage && matchesExperience && matchesScore
        );
      },
    );
  }, [filters]);
            ...
            {filteredCandidates.length === 0 ? (
              ...
              filteredCandidates.map((candidate, index) => (
                <tr key={candidate.id} className="sm:text-sm text-xs">
                  ...
                  <td className="text-nowrap">{candidate.currentRole}</td>
                  <td className="text-nowrap">{candidate.currentCompany}</td>
                  <td className="text-nowrap">{candidate.appliedFor}</td>
                  ...
                  <td className="text-nowrap">{candidate.lastActivity}</td>
                  <th>
