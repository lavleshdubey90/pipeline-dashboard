...
import { MapPin, Building2, Clock, Users, CheckCircle2, Target, Share2, Briefcase, X } from 'lucide-react';
            ...
            <div className="hero min-h-[300px]">
                <div className="hero-content text-center">
                    <div>
                        ...
                        <p className="text-base-content/60">The job you're looking for doesn't exist.</p>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="space-y-4">
                ...
                <div className="flex items-center gap-2 mb-1">
                    <div className='w-full flex items-start justify-between'>
                        <div className='flex-1 flex-col gap-1 flex w-full!'>
                            ...
                            <div className={`badge badge-sm capitalize font-medium ${job.status === 'active' ? 'badge-success' :
            ...
            <div className="bg-base-100 rounded-lg p-3">
                ...
                <p className="text-base-content leading-relaxed mt-1">{job.description}</p>
            </div>
            ...
            <div className="bg-base-100 rounded-lg p-3">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                    {job.skills.map((skill, index) => (
                        <span key={index} className="badge badge-sm badge-primary badge-soft font-medium">
                            {skill}
                        </span>
                    ))}
                </div>
            ...
            <div className="bg-base-100 rounded-lg p-3">
                ...
                <div className="space-y-2">
                    {job.requirements.map((req, index) => (
                        <div key={index} className="flex items-center gap-2 text-sm justify-start flex-row text-base-content">
                            <div aria-label="success" className="status status-success" /> {req}
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="bg-base-100 rounded-lg p-3">
                ...
                <div className="space-y-2">
                    {job.responsibilities.map((resp, index) => (
                        <div key={index} className="flex items-center gap-2 text-sm justify-start flex-row text-base-content">
                            <div aria-label="accent" className="status status-accent" /> {resp}
                        </div>
                    ))}
                </div>
            </div>
                        ...
                        return (
                            <div key={index} className={`step step-${stage.color} relative`}>
                                <div className="flex items-center justify-between w-full pl-2">
                                    ...
                                    <span className={`badge badge-sm badge-${stage.color} badge-soft`}>
                                        {stage.count}
                                    </span>
                                </div>
                            </div>
